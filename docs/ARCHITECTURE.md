...
A production-grade ALM/treasury risk engine demo. **Out of scope:** FRTB / market risk capital. **In scope:** IRRBB under BCBS 368 / EBA Guidelines, liquidity (LCR, NSFR, ALMM survival horizon), ICAAP-flavored capital adequacy, behavioral modeling and FTP for NII.
...
| UI               | Streamlit                           | Existing: kept, repointed at DuckDB / risk_outputs         |
| Deferred         | FastAPI, Docker Compose, GH Actions | Future upgrade (not done)                                  |
...
├── basel_risk_engine/             # rate models, behavioral, EVE/NII, FTP, liquidity
├── src/                           # legacy compute.py / queries.py / scenario.py 
├── dbt_project/                   # core of the dbt project
│   ├── dbt_project.yml
│   ├── profiles.yml
│   └── models/
│       ├── staging/
...
│           ├── alm/
│           └── capital/
├── basel_dagster/                 # Flat layout (no dagster_project/ wrapper)
│   ├── assets/
│   │   ├── ingestion.py
│   │   └── transformations.py
│   ├── jobs.py
...
With the dbt_project module the risk engine and dbt absorb the SQL-shaped compute, `src/` shrinks to dashboard glue (or empties entirely) and `compute.py` becomes a thin client over `basel_risk_engine` + dbt marts.
...
- IRRBB done properly: not bps shocks on hardcoded gaps, but a **calibrated short-rate model** driving MC curve paths, with behavioral overlays (NMD repricing assumptions, embedded optionality) and EVE distributions. The EBA supervisory outlier test is computed from a distribution, not a point.
